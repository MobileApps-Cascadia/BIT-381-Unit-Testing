...
final class Unit_TestingUITests: XCTestCase {
    ...
    func testUITN() throws {
        let app = XCUIApplication()
        app.launch()
        let inputTextField = app.textFields["Type your name here:"]
        inputTextField.tap()
        inputTextField.typeText("Frank Dilbert")
        
        let nStaticText = app.staticTexts["UserOutput"].value as! String
        print("FOUND: "   + nStaticText + " ===========================================================================================")

        XCTAssertEqual(inputTextField.label, nStaticText, "Expected 'UserOutput' to display 'Frank Dilbert'")
    }
}
